...
\begin{document}
  \section{プラズマの温度}
    ...
    粒子の速度分布はMaxwell分布\index{まくすうぇる分布@Maxwell分布}に従う．単位体積当たり$n$個の粒子が温度$T$で熱平衡状態に
    ...
    \end{align}
    である．これは$v = v_{\r{th}}$で最大となる．また，速度ベクトルの向きに依らず，速さのみに依存する．\textbf{等方分布}\index{とうほうぶんぷ@等方分布}である．
    \par
  ...
  \section{電気的中性}
    ...
    プラズマは以下の機構により\textbf{準中性}\index{じゅんちゅうせい@準中性}と呼ばれる．
    まず，プラズマはランダムな熱運動をしている．
    この熱運動により，ある瞬間には揺動電場\index{ようどうでんば@揺動電場}が形成される．
  ...
  \section{Debye遮蔽}
    ...
    プラズマによる遮蔽を考えたものとして，次の\textbf{Debyeポテンシャル}\index{でばいポテンシャル@Debyeポテンシャル}がある．
    \begin{align}
      \phi(r) = \frac{1}{4\pi\epsilon_0}\frac{q}{r}\exp\qty(-\frac{r}{\lambda_{\r{D}}})
    \end{align}
    $\lambda_{\r{D}}$を\textbf{Debye遮蔽長}\index{でばいしゃへいちょう@Debye遮蔽長}，$\exp(-r/\lambda_{\r{D}})$を\textbf{Debye遮蔽ファクター}\index{でばいしゃへいふぁくたー@Debye遮蔽ファクター}という．
    ...
    が得られる．
  \section{プラズマ・パラメータ}
    Debyeの遮蔽長を半径とする球(Debye球)\index{でばいきゅう@Debye球}を考える．
    ...
    \end{align}
    という条件を満たさなければならない．よって，この$N_\lambda$をプラズマ・パラメータ\index{ぷらずまぱらめーた@プラズマ・パラメータ}という．
    ...
    \end{align}
    となる．
    以下に，\refe{plasma-parameter}と\refe{plasma-parameter2}が等価であること，すなわち，
  ...
  \section{プラズマ振動}
    ...
    \end{align}
    である単振動の方程式であることがわかる．
    この$\omega_p$をプラズマ振動数\index{ぷらずましんどうすう@プラズマ振動数}という．
