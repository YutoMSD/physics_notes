...
\begin{document}
  ...
  \subsection{Eulerの運動方程式}
    ...
    \end{align}
    これらを\refe{eq:momentum4}に代入して整理すると
    \begin{align}
      mn\pdv{\bm{u}}{t} + m\bm{u} \qty[\pdv{n}{t} + \nabla \cdot (n\bm{u})] + mn(\bm{u} \cdot \nabla)\bm{u} = \bm{F}^{\r{ext}} - \nabla p
    \end{align}
    $[]$内は密度連続の式より0である．
    よって，\textbf{Eulerの運動方程式}\index{Eulerのうんどうほうていしき@Eulerの運動方程式}が得られる．
    \begin{align}
...
\section{流体のエネルギー方程式}
  ...
  \end{align}
  と表される．また，\refe{eq:energy}は流体の運動エネルギー密度の時間変化と考えられる．つまり，外力，圧力勾配が無い場合には流体の運動エネルギー密度は保存される．
